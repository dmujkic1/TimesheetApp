      ...
      <div class="relative flex h-16 items-center justify-between">
        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
          <div class="flex shrink-0 items-center">
            <img class="h-9 w-auto" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZH1SBB2-ku_kiJhUiWa5vvjeHpavjbds6jA&s?color=indigo&shade=500" alt="Your Company" />
          </div>
              ...
              <MenuItems class="absolute right-0 z-10 mt-2 w-72 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5 focus:outline-hidden max-h-96 overflow-auto">
                <MenuItem v-for="(notification, index) in notifications" :key="index" v-slot="{ active }">
                  <div :class="[active ? 'bg-gray-100' : '', 'px-4 py-2 text-sm text-gray-700 cursor-pointer']">
                    <p class="font-semibold">{{ notification.notes  }}</p>
                  </div>
                </MenuItem>
...
const getCurrentRoute = () => {
  ...
  if (pathname.includes('employees/getAll')) {
    ...
    return 'Timesheets na provjeri';
  } else if (pathname.includes('reporting/hours-per-user')) {
    return 'Izvještaj sati';
...
const navigation = [
  ...
  { name: 'Izvještaj sati', href: '/reporting/hours-per-user', current: getCurrentRoute() === 'Izvještaj sati' },
