              ...
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Datum početka</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Datum završetka</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Napomena</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  <tr v-for="entry in pendingEntries.data" :key="entry.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ formatDate(entry.start_date) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ formatDate(entry.end_date) }} <!-- DATUM ZAVRŠETKA --></td>
...
router.visit(route('manager.ooos.pending', {
    page: page,
  }), {
  preserveState: true,
  preserveScroll: true,
...
const approveEntry = (id) => {
  if (!confirm("Da li ste sigurni da želite odobriti ovaj unos?")) return;

  router.put(route('manager.ooos.approveEntry', id), {}, {
    onSuccess: () => {
      // osvježi podatke nakon odobravanja
      router.visit(route('manager.ooos.approveEntry'), {
        preserveScroll: true,
      });
    },
    onError: () => {
      alert('Greška prilikom odobravanja unosa.');
    }
  });
}
...
const submitRejection = () => {
  ...
  })).put(route('manager.ooos.rejectEntry', { ooo: entryToReject.value.id }), {
    preserveScroll: true,
